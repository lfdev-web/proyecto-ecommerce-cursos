...
class Command(BaseCommand):
    ...
    def _seed_promociones(self, courses):
        ...
        contenido desde el primer arranque.

        Se delega en `crear_promociones` por lo mismo que las actividades: ese
        comando también se corre suelto en el servidor (las ofertas vencen y hay
        que renovarlas), y las dos rutas deben dejar el catálogo idéntico.
        """
        from django.core.management import call_command
        call_command('crear_promociones', verbosity=0)
        en_oferta = Course.objects.exclude(promo_price=None).count()
        self.stdout.write(f'  {en_oferta} cursos en oferta (carrete de la portada)')
